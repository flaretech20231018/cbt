...
```bash
# プロジェクト全体をスキャン
docker compose exec cbt npx react-doctor@latest .

# 詳細表示
docker compose exec cbt npx react-doctor@latest . --verbose

# 差分スキャン（PR 単位での確認に使う）
docker compose exec cbt npx react-doctor@latest . --diff main
```
