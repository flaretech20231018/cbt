...
| フェーズ | 名称 | 状態 | 主な目的 |
|---------|------|------|---------|
| Phase 0 | ドキュメント整備 | ★ 進行中 | 設計・仕様・開発ルールを文書化し、実装の迷いをなくす |
| Phase 1 | プロジェクト基盤構築 | 未着手 | Next.js・Docker・テスト基盤・CI を整備し、「動く土台」を作る |
...
| Vitest | `vitest.config.ts` + React Testing Library セットアップ。`npx vitest run` が通る状態 |
| Storybook | `.storybook/` セットアップ。`storybook dev` と `storybook build` が動く状態 |
| Playwright | `playwright.config.ts` セットアップ。`npx playwright test` が通る状態（テスト 0 件） |
| CI | `.github/workflows/ci.yml`（lint / type-check / test / storybook / playwright / build）を作成 |
...
- [ ] `npm run build` が成功する
- [ ] `npx vitest run` がエラーなしで通る
- [ ] `storybook build` が成功する
- [ ] `npx playwright test` がエラーなしで通る
- [ ] GitHub Actions の CI 全ジョブが green になる
- [ ] PR を作成したとき CodeRabbit がコメントを付与する

### 使用技術・ツール

`Next.js 15` `TypeScript` `Tailwind CSS` `ESLint` `Prettier` `Docker` `GitHub Actions` `Vitest` `React Testing Library` `Storybook` `Playwright` `CodeRabbit`
...
> フェーズ開始時に Epic を詳細 Issue に展開する際の参考。そのまま使う必要はなく、前フェーズの学びを反映して調整すること。
...
| 書くもの | テストレイヤー（単体 / 統合 / E2E）ごとの対象・ツール・方針 ＋ Storybook Stories の設計 ＋ E2E テストケース設計（Playwright） |
| ポイント | 機能要件の各チェックボックスに対応するテストが最低 1 つ存在することを確認する。E2E テストは主要ユーザーフロー（サインイン → 記録作成 → 一覧確認 → ログアウト等）を対象とする |

**◆ セルフレビューゲート**: 機能要件チェックリストのすべての項目に対応するテストが計画されているか？ テストの粒度は適切か？ E2E テストで検証すべき主要フローが網羅されているか？
...
```
6-B-1. Step 4 のコンポーネント構成に従い、画面仕様を満たすコンポーネントを実装する
         → Server Component: データ取得・認証チェック
         → Client Component: フォーム状態管理（React Hook Form）・ユーザー操作
6-B-2. Storybook Stories を作成し、各状態（空・入力中・エラー・送信中）を視覚的に確認する
6-B-3. RTL で統合テストを書く（フォーム操作・エラー表示・Server Action 呼び出し）
6-B-4. 画面遷移・レスポンシブ対応を確認する
6-B-5. 次の画面に進む（6-B-1 へ戻る）
```

##### 6-C. E2E テスト・品質確認

```
6-C-1. Step 5 で設計した E2E テストケースを Playwright で実装する
6-C-2. `npx playwright test` で主要フローが通ることを確認する
6-C-3. `npx vitest --coverage` でカバレッジを確認し、未カバー箇所を補強する
```

> **6-A → 6-B → 6-C の順にする理由**: Server Actions が安定した Contract を提供してからコンポーネントを組み立て、
> 最後に E2E テストでユーザーフロー全体を検証する。フロントエンドの実装中に「API の入出力が変わった」という手戻りを防ぐ。
...
- [ ] Server Actions の単体テスト（Vitest）が存在し、パスしている
- [ ] レスポンシブ対応：スマートフォン・PC の両方で崩れない
- [ ] Storybook Stories が `components/ui/` の全コンポーネントに存在する
- [ ] E2E テスト（サインイン → 記録作成 → 一覧確認 → ログアウト）がパスする
- [ ] 初回でも 3 分以内に 1 記録を作成できる UX になっている（KPI）

### 使用技術・ツール

`React Hook Form` `Zod` `Supabase JS` `Server Actions` `Vitest` `React Testing Library` `Storybook` `Playwright`
...
- [ ] 各コンポーネントの Storybook Stories 作成

**完了定義:**
- [ ] 全コンポーネントが WCAG AA 準拠（コントラスト比・タッチターゲット 44px）
- [ ] 各コンポーネントに Storybook Story が存在する
- [ ] `npx storybook build` が成功する
...
- [ ] Step 5: テスト方針（Storybook Stories 設計・RTL テスト計画）
...
**完了定義:**
- [ ] `npx vitest run` で create-record.test.ts が全テストパス
...
#### feat: 記録作成フォーム UI 実装 + Storybook + RTL テスト（SDD Step 6-B）
...
- [ ] Storybook Stories（Empty / Filled / WithFieldError / WithFormError / Submitting）
- [ ] RTL 統合テスト（送信→createRecord 呼出、全空送信→エラー表示）
...
#### feat: 記録編集フォーム UI 実装 + Storybook + RTL テスト（SDD Step 6-B）
...
#### test: E2E テスト実装（主要ユーザーフロー）

**Labels:** type: test, priority: must
**依存:** 全機能実装完了

**テストケース:**
- [ ] フロー 1: サインアップ → 記録作成（3コラム入力）→ 一覧で確認 → ログアウト
- [ ] フロー 2: ログイン → 一覧 → 詳細 → 編集 → 保存確認
- [ ] フロー 3: ログイン → 一覧 → 詳細 → 削除 → 一覧で削除確認
- [ ] フロー 4: 未ログインで /records → /sign-in にリダイレクト

**完了定義:**
- [ ] `npx playwright test` で全テストケースパス
- [ ] GitHub Actions CI に Playwright ジョブ追加
...
| 動作確認 | 本番 URL でサインアップ → 記録作成 → ログアウトの一連フローを確認 |
...
#### test: 本番環境 E2E フロー確認 + リリースチェックリスト

**Labels:** type: test, priority: must

**確認フロー:**
- [ ] 本番 URL でサインアップ → 記録作成 → 一覧確認 → ログアウト
- [ ] 再ログイン → 詳細 → 編集 → 削除
...
> テスト基盤・Storybook・CI は Phase 1 で整備済みのため、Phase 3 は初日から TDD / SDD サイクルで開発できる。
