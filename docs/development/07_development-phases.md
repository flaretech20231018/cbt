...
| CI | `.github/workflows/ci.yml`（lint / type-check / test / playwright / build）を作成 |
...
`Next.js 15` `TypeScript` `Tailwind CSS` `ESLint` `Prettier` `Docker` `GitHub Actions` `Vitest` `React Testing Library` `Playwright` `CodeRabbit`
...
| 書くもの | テストレイヤー（単体 / 統合 / E2E）ごとの対象・ツール・方針 ＋ E2E テストケース設計（Playwright） |
...
6-B-2. RTL で統合テストを書く（フォーム操作・エラー表示・Server Action 呼び出し）
6-B-3. 画面遷移・レスポンシブ対応を確認する
6-B-4. 次の画面に進む（6-B-1 へ戻る）
```
...
`React Hook Form` `Zod` `Supabase JS` `Server Actions` `Vitest` `React Testing Library` `Playwright`
