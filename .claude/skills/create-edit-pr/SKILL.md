...
## Step 1. Issue の完了定義をチェックする

ブランチ名から Issue 番号を特定し、完了定義をすべて満たしているか確認する。

```bash
# ブランチ名から Issue 番号を取得
git branch --show-current   # 例: feat/7/add-login → Issue #7

# Issue の完了定義を確認
gh issue view <Issue番号>
```

- 完了定義がすべて満たされている → 次のステップへ進む
- 未達の項目がある → ユーザーに報告し、対処してから PR 作成に進む
- Issue 番号が不明・Issue なし → このステップをスキップする
